...
with open(f"data/he4/imsrg2/data.json") as f:
    data = json.load(f)

# Top left
ham = "NN-only"
for basis in ["HF", "truncated NAT"]:
    hws = []
    energies = []
    ...
    if basis == "truncated NAT":
        basis = "NAT"
    ax1.plot(hws, energies, "o-", label=basis)
    hws_ref_line = [18, 34]
    energies_ref_line = [-28.4499] * 2
    ax1.plot(hws_ref_line, energies_ref_line, "k-.")
...
for basis in ["HF", "truncated NAT"]:
    ...
    if basis == "truncated NAT":
        basis = "NAT"
    ax2.plot(hws, energies, "o-", label=basis)
    hws_ref_line = [18, 34]
    energies_ref_line = [-29.1125] * 2
    ax2.plot(hws_ref_line, energies_ref_line, "k-.")
...
for basis in ["HF", "truncated NAT"]:
    ...
    if basis == "truncated NAT":
        basis = "NAT"
    ax3.plot(hws, radii, "o-", label=basis)
...
for basis in ["HF", "truncated NAT"]:
    ...
    if basis == "truncated NAT":
        basis = "NAT"
    ax4.plot(hws, radii, "o-", label=basis)
...
path = "he4_imsrg2_results_with_NCSM.pdf"
